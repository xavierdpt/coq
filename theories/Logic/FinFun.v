...
Qed.
    ...
    Proof.
assert (lem1:=f2n_n2f).
assert (lem2:=restrict_n2f).
assert (lem3:=n2f_ext).
assert (lem4:=f2n_inj).
assert (lem5:=f2n_ok).
assert (lem6:=restrict_f2n).
split.
{
intro hinj.
red in hinj.
...
assert (lem1y:=lem1).
clear lem1.
specialize (lem1x _ _ hx).
specialize (lem1y _ _ hy).
rewrite <- lem1x;clear lem1x.
rewrite <- lem1y;clear lem1y.
apply f_equal.
apply hinj. clear hinj.
assert (lem2x := lem2).
assert (lem2y := lem2).
clear lem2.
specialize (lem2x n f h x hx).
specialize (lem2y n f h y hy).
rewrite lem2x. rewrite lem2y.
clear lem2x lem2y.
generalize (h x hx).
generalize (h y hy).
rewrite heq.
intros hy1 hy2.
erewrite lem3. reflexivity.
}
{
intro hinj.
red in hinj.
...
reflexivity.
  ...
  Proof.
    intros h.
    assert (lem1:=restrict_injective).
    assert (lem2:=restrict_surjective).
    assert (lem3:=Endo_Injective_Surjective).
    assert (lem4:=Fin_Finite).
    assert (lem5:=@Fin.eq_dec).
    specialize (lem1 _ _ h).
    specialize (lem2 _ _ h).
    destruct lem1 as [ lem1l lem1r ].
    destruct lem2 as [ lem2l lem2r ].
    edestruct lem3 as [ lem3l lem3r ].
    shelve. shelve. clear lem3.
    split.
    {
      intro hinj.
      apply lem2l. clear lem2l lem2r.
      apply lem3l. clear lem3l lem3r.
      apply lem1r. clear lem1l lem1r.
      assumption.
      Unshelve.
      apply lem4.
      red.
    ...
    {
      intro hsurj.
      apply lem1l. clear lem1l lem1r.
      apply lem3r. clear lem3l lem3r.
      apply lem2r. clear lem2l lem2r.
      assumption.
    }
  Qed.
